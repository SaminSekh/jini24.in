...
function setupSheets() {
  ...
  if (usr.getLastRow() === 0) {
    usr.appendRow(["Email","Name","Phone","Address","SelectedArea","Lat","Lng","State","District","City"]);
  }
  ...
  if (contentSheet.getLastRow() === 0) {
    ...
    contentSheet.appendRow(["allow_guest_booking","true"]);
  } else {
    ...
    if (!existingKeys.includes("allow_guest_booking"))  contentSheet.appendRow(["allow_guest_booking","true"]);
  }
  ...
  try {
    ...
    switch (action) {
      ...
      case "getBookingByEmail":
        ...
        const foundByEmail = allBookings.filter(b => (b.CustomerEmail || "").toLowerCase().trim() === email);
      ...
      case "getWishesByEmail":
        const wishEmail = (e.parameter.email || "").toLowerCase().trim();
        const allWishes = getSheetData(ss, SHEET_WISHES);
        const foundWishes = allWishes.filter(w => (w.CustomerEmail || "").toLowerCase().trim() === wishEmail);
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case "deleteUser":
        return jsonResponse(deleteRow(ss, SHEET_USERS, "Email", payload.email));
