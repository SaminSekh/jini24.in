...
function initAdminSidebar() {
  ...
  document.addEventListener("click", (e) => {
    if (!sidebar || !sidebar.classList.contains("show-mobile")) return;
...
function refreshAdminView() {
  ...
  if (view === "dashboard") {
    ...
    renderFrontendView();
  } else if (view === "sitesetting") {
    renderSiteSettingsView();
  } else if (view === "admins") {
    renderAdminsView();
  } else if (view === "users") {
    renderUsersManager();
...
function updateAdminBadges() {
  ...
  // Job Bookings pending badge
  const pendingJobsCount = bookings.filter(b => b.JobStatus === "Pending").length;
  const jobsBadge = document.getElementById("pending-badge");
  if (jobsBadge) {
    jobsBadge.textContent = pendingJobsCount;
  }

  // Extra Wishes pending badge
  const pendingWishesCount = wishes.filter(w => w.JobStatus === "Pending").length;
  const wishesBadge = document.getElementById("pending-wishes-badge");
  if (wishesBadge) {
    wishesBadge.textContent = pendingWishesCount;
  }
  ...
  const payBadge = document.getElementById("pending-payment-badge");
  if (payBadge) {
    payBadge.textContent = pendingPayCount;
    payBadge.style.display = pendingPayCount > 0 ? "inline-block" : "none";
  }
}
...
function renderDashboard() {
  ...
  const pendingJobs      = bookings.filter(b => b.JobStatus === "Pending").length;
  const pendingWishes    = wishes.filter(w => w.JobStatus === "Pending").length;
  const pendingPayments  = [...bookings, ...wishes].filter(b => b.PaymentStatus === "Pending_Payment").length;
  ...
  const verifiedUsers    = users.filter(u => u.Email).length;
  ...
  setMetric("metric-pending-jobs", pendingJobs);
  setMetric("metric-pending-wishes", pendingWishes);
  setMetric("metric-pending-payments", pendingPayments);
  setMetric("metric-active", activeBookings);
  setMetric("metric-completed", completedJobs);
  setMetric("metric-users", verifiedUsers);
...
// ══════════════════════════════════════════════════════════
function initSiteSettingsModule() {
  document.getElementById("save-sitesetting-btn")?.addEventListener("click", async () => {
    const btn = document.getElementById("save-sitesetting-btn");
    Utils.showLoading(btn, "Saving...");
    try {
      const inp = document.getElementById("setting-allow-guest-booking");
      const updates = [
        {
          Key: "allow_guest_booking",
          Value: inp ? String(inp.checked) : "true"
        }
      ];
      await SheetsAPI.updateContent(updates);
      
      // Update local content state while merging existing keys
      let currentContent = AppState.get("content") || [];
      updates.forEach(u => {
        const idx = currentContent.findIndex(c => c.Key === u.Key);
        if (idx > -1) currentContent[idx] = u;
        else currentContent.push(u);
      });
      AppState.set("content", currentContent);
      
      Toast.show("✅ Site settings saved successfully!", "success");
      renderSiteSettingsView();
    } catch (err) {
      Toast.show("Error saving site settings: " + err.message, "error");
    } finally {
      Utils.hideLoading(btn);
    }
  });
...
function renderSiteSettingsView() {
  const content = AppState.get("content") || [];
  const guestItem = content.find(c => c.Key === "allow_guest_booking");
  const guestCb = document.getElementById("setting-allow-guest-booking");
  if (guestCb) {
    // Default is true if not explicitly saved as false
    const isAllowed = guestItem ? String(guestItem.Value).toLowerCase() === "true" : true;
    guestCb.checked = isAllowed;
  }
}
...
// USERS MANAGEMENT
// ══════════════════════════════════════════════════════════
window.renderUsersManager = function renderUsersManager() {
  const users = AppState.get("users") || [];
  ...
  if (searchVal) {
    filtered = filtered.filter(u => {
      ...
      return email.includes(searchVal) ||
  ...
  // Apply Sorting
  const sortVal = document.getElementById("admin-users-sort")?.value || "az";
  filtered = [...filtered];
  if (sortVal === "az") {
  ...
  const totalPages = Math.ceil(filtered.length / window.usersPageSize) || 1;
  if (window.usersPage > totalPages) window.usersPage = totalPages;
  const start = (window.usersPage - 1) * window.usersPageSize;
  const paginated = filtered.slice(start, start + window.usersPageSize);
  ...
  if (paginated.length === 0) {
    ...
    paginated.forEach(u => {
      ...
      tr.innerHTML = `
        <td><span style="font-weight:600;color:var(--slate-800)">${u.Email || 'N/A'}</span></td>
        <td>${u.Name || 'N/A'}</td>
        <td>${u.Phone || 'N/A'}</td>
        <td>${u.State || 'N/A'}</td>
        ...
        <td>
          <div style="display:flex;gap:8px">
            <button class="btn btn-sm btn-secondary" onclick="openAdminUserModal('${u.Email}')" title="Edit User">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 20h9M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
            </button>
            <button class="btn btn-sm btn-secondary" style="color:var(--red-500);border-color:var(--red-200);background:var(--red-50)" onclick="deleteAdminUser('${u.Email}')" title="Delete User">
  ...
  if (paginationContainer) {
    paginationContainer.innerHTML = `
      <div style="font-size:0.875rem;color:var(--slate-500)">
        Showing <strong>${start + 1}-${Math.min(start + window.usersPageSize, filtered.length)}</strong> of <strong>${filtered.length}</strong> users
      </div>
      <div style="display:flex;gap:12px;align-items:center">
        <select class="form-control" style="width:auto;padding:4px 8px;font-size:0.875rem" onchange="window.usersPageSize=parseInt(this.value); window.usersPage=1; renderUsersManager()">
          <option value="10" ${window.usersPageSize === 10 ? 'selected' : ''}>10 per page</option>
          <option value="50" ${window.usersPageSize === 50 ? 'selected' : ''}>50 per page</option>
          <option value="100" ${window.usersPageSize === 100 ? 'selected' : ''}>100 per page</option>
        </select>
        <div style="display:flex;gap:4px">
          <button class="btn btn-secondary btn-sm" ${window.usersPage === 1 ? 'disabled' : ''} onclick="window.usersPage--; renderUsersManager()">Prev</button>
          <button class="btn btn-secondary btn-sm" ${window.usersPage === totalPages ? 'disabled' : ''} onclick="window.usersPage++; renderUsersManager()">Next</button>
        </div>
      </div>
    `;
  }
...
window.openAdminUserModal = function openAdminUserModal(email) {
  const users = AppState.get("users") || [];
  ...
  document.getElementById("admin-user-state").value = user.State || "";
  document.getElementById("admin-user-district").value = user.District || "";
  document.getElementById("admin-user-city").value = user.City || "";
  
  Modal.open("admin-user-edit-modal");
}

window.saveAdminUser = async function() {
  const btn = document.querySelector("#admin-user-edit-form button[type='submit']");
  ...
  const payload = {
    ...
    data: {
      ...
      State: document.getElementById("admin-user-state").value,
      District: document.getElementById("admin-user-district").value,
  ...
  try {
    ...
    if (result.status === "ok") {
      ...
      Modal.close("admin-user-edit-modal");
      // Optionally re-fetch users or update in AppState
      let users = AppState.get("users") || [];
      const idx = users.findIndex(u => u.Email === payload.data.Email);
      if(idx !== -1) {
        users[idx] = { ...users[idx], ...payload.data };
        AppState.set("users", users);
        if (currentAdminView === "users") renderUsersManager();
      } else {
        await SheetsAPI.fetchInitialData();
        if (currentAdminView === "users") renderUsersManager();
      }
    } else {
...
window.exportUsers = function() {
  ...
  exportToCSV(filtered, `Registered_Users_Export_${new Date().toISOString().slice(0,10)}.csv`);
};

// ══════════════════════════════════════════════════════════
// UNIVERSAL TABLE HEADER SORTING (A-Z / Z-A / 0-9)
// ══════════════════════════════════════════════════════════
function initUniversalTableSorting() {
  ...
  function attachSortingToTable(table) {
    ...
    headers.forEach((th, colIdx) => {
      ...
      // Exclude Action, Photo, Image, QR Code, etc.
      if (
      ...
      if (!th.dataset.sortBound) {
        ...
        th.addEventListener("click", () => {
          ...
          // Set active header arrow
          th.dataset.sortDir = newDir;
          if (arrow) {
            arrow.innerHTML = newDir === "asc" ? " ▲" : " ▼";
            arrow.style.color = "#10b981"; // Highlight active sort in green
            arrow.style.opacity = "1";
          }

          // Sort tbody rows directly
          const tbody = table.querySelector("tbody");
          if (!tbody) return;

          const rows = Array.from(tbody.querySelectorAll("tr"));
          // Filter out loading / empty state rows
          const validRows = rows.filter(r => r.children.length > 1 && !r.querySelector("td[colspan]"));
          ...
          validRows.sort((rowA, rowB) => {
            ...
            // Helper to clean price/currency/numeric strings
            ...
            let cmp = 0;
            if (numA !== null && numB !== null && !/[a-zA-Z]{3,}/.test(cellA) && !/[a-zA-Z]{3,}/.test(cellB)) {
              cmp = numA - numB;
            } else {
  ...
  // Scan current tables
  container.querySelectorAll("table").forEach(attachSortingToTable);

  // MutationObserver for dynamic updates (search, pagination, view switching)
  if (!window._adminTableSortObserver) {
    window._adminTableSortObserver = new MutationObserver(() => {
      container.querySelectorAll("table").forEach(attachSortingToTable);
    });
    window._adminTableSortObserver.observe(container, { childList: true, subtree: true });
  }
}
