...
function isGuestBookingAllowed() {
  const content = AppState.get("content") || [];
  const item = content.find(c => c.Key === "allow_guest_booking");
  if (!item) return true;
  return String(item.Value).toLowerCase() === "true";
}

function openBookingModal(serviceId, serviceName) {
  if (!isGuestBookingAllowed() && AppState.get("userRole") === "Guest") {
...
window.bookingSubmit = async function() {
  ...
  try {
    const profile = AppState.get("userProfile");
    const customerEmail = (profile && profile.email) ? profile.email : (email || "");
...
function initExtraPage() {
  ...
  if (!isGuestBookingAllowed() && (role === "Guest" || !profile)) {
    authOverlay.classList.remove("hidden");
  ...
  const guestFields = document.getElementById("wish-guest-fields");
  const guestName = document.getElementById("wish-name");
  const guestEmail = document.getElementById("wish-email");
  ...
  if (!profile) {
    ...
    if (guestName) { guestName.required = false; guestName.value = profile.name || ""; }
    if (guestEmail) { guestEmail.required = false; guestEmail.value = profile.email || ""; }
  }
  ...
  if (form && !form.dataset.bound) {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        const guestNameVal = document.getElementById("wish-name")?.value.trim() || "";
        const guestEmailVal = document.getElementById("wish-email")?.value.trim() || "";

        const customerName = profile ? profile.name : (guestNameVal || "Customer");
        const customerEmail = profile ? profile.email : guestEmailVal;
        ...
        Toast.show("✨ Wish placed successfully! You can track it in your orders after signing in.", "success");
        form.reset();
        resetWishSteps();
...
window.nextWishStep = function(currentStep) {
  ...
  if (currentStep === 2) {
    const emailInput = document.getElementById("wish-email");
    if (emailInput && emailInput.offsetParent !== null && !emailInput.checkValidity()) {
