...
const Router = (() => {
  ...
  function showPage(pageName) {
    ...
    // Hide bottom navigation for Admin Panel
    const bottomNav = document.querySelector(".bottom-nav");
    if (bottomNav) {
      bottomNav.style.display = pageName === "admin" ? "none" : "";
    }
...
// ── App Install Popup ─────────────────────────────────────────
document.addEventListener("dataLoaded", () => {
  // Do not show popup for admins
  if (AppState.get("userRole") === "Admin") return;

  // Check if the app is already installed and running in standalone mode (PWA)
  const isStandalone = window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true;
  
  if (!isStandalone) {
    setTimeout(() => {
      Modal.open("app-install-popup");
    }, 1500); // Show shortly after data load
  }
});

document.addEventListener("DOMContentLoaded", () => {
  const downloadBtn = document.getElementById("btn-download-app-popup");

  if (downloadBtn) {
    downloadBtn.addEventListener("click", () => {
      Modal.close("app-install-popup");
      
      const content = AppState.get("content") || [];
      const appLinkItem = content.find(c => c.Key === "app_link");
      const playstoreLink = appLinkItem ? appLinkItem.Value : "";

      if (playstoreLink && playstoreLink.trim() !== "") {
        ...
        if (window._pwaInstallPrompt) {
          ...
          Toast.show("App is already installed or your browser requires manual installation from the menu.", "info");
        }
      }
    });
  }
